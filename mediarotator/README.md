...
- **Seedbox Sync**: Rsync media from a seedbox staging area with optional delayed cleanup
...
export SEEDBOX_SYNC_ENABLED="true"
export SEEDBOX_HOST="seedbox.example.com"
export SEEDBOX_USER="seeduser"
export SEEDBOX_PORT="22"
export SEEDBOX_MOVIES_PATH="~/media/movies"
export SEEDBOX_TV_PATH="~/media/tv"
export SEEDBOX_MOVIES_DEST="/mnt/netstorage/Media/RotatingMovies"
export SEEDBOX_TV_DEST="/mnt/netstorage/Media/RotatingTV"
export SEEDBOX_CLEANUP_DELAY_HOURS="12"
export SEEDBOX_SSH_KEY_PATH="/root/.ssh/id_rsa"
export SEEDBOX_SSH_STRICT_HOST_KEY_CHECKING="accept-new"
export SEEDBOX_SSH_EXTRA_ARGS=""
```
...
MediaRotator can rsync validated downloads from a seedbox staging area (e.g., `~/media/movies` and `~/media/tv`)
into the local rotating library paths. If enabled, it will optionally delete seedbox files after a delay
(default 12 hours).

Notes:
- The container needs SSH access to the seedbox (mount your SSH keys into `/root/.ssh`).
- Local destination paths must be mounted into the container (see `docker-compose.yml`).
- Use `--skip-sync` to disable sync for a run, or `--sync-only` to run sync without add/rotate.
